...
const initializePassport =() => {
        ...
        new LocalStrategy ({passReqToCallback: true, usernameField: "email" },
            async (req, username, password, done) => {
                try {
                    ...
                    const { firstName, lastName, email, age } = req.body; // tomamos la data del user desde el body
                    const user = await usersDao.getByEmail(username); // buscamos por email el usuario
                    ...
                    const createUser= await usersDao.create(newUser); // se utilizan los datos de "newUser" para crear un nuevo usuario
    ...
    passport.use( // se crea la estrategia para autenticarse con google
        ...
        new GoogleStrategy(
            {
                clientID:"", // ID proporcionado por google
                clientSecret:"", // Clave proporcionada por google
            ...
            async (accessToken, refreshToken, profile, cb) => {
                try {
                    ...
                    const existUser = await usersDao.getByEmail(emails[0].value)
                    ...
                    const newUser = await usersDao.create(user); // en caso de que no exista crea el usuario con la información que nos brindó google

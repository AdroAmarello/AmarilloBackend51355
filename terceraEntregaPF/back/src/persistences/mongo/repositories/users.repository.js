...
const getUsers = async (query, options) => { // creamos una función para traer todo
    const users = await usersModel.paginate(query, options); // utilizamos el método paginate para traer los usuarios según parámetros y para obtener los datos de total de páginas, el número de página, etc.
    return users;
};
